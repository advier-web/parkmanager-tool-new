...
export default function MobilitySolutionsPage() {
  ...
  const getPickupPreferenceMatch = useCallback((solution: MobilitySolution): boolean => {
    ...
    // Check if solution supports the user's preference
  ...
  const sortSolutionsByScore = useCallback((
  ...
  ): { solution: MobilitySolution, score: number, trafficMatch: number, pickupMatch: boolean, contributingReasons: { [reasonId: string]: number } }[] => {
    if (!solutions) return [];
    ...
    scoredSolutions.sort((a, b) => {
      // First priority: pickup preference match (matching solutions first)
      if (a.pickupMatch !== b.pickupMatch) return b.pickupMatch ? 1 : -1;
      // Second priority: traffic type match
      if (b.trafficMatch !== a.trafficMatch) return b.trafficMatch - a.trafficMatch;
      // Third priority: reason-based score
  ...
  }, [mobilitySolutions, reasons, businessParkInfo.trafficTypes, businessParkInfo.employeePickupPreference, activeFilters, calculateScoreForSolution, getTrafficTypeMatchScore, getPickupPreferenceMatch, getReasonScores, sortSolutionsByScore]); // Added dependencies

  // Get solutions for comparison (either selected ones or top scoring ones)
  const getComparisonSolutions = useCallback((): { solutions: MobilitySolution[], contributingReasons: { [solutionId: string]: { [reasonId: string]: number } } } => {
    const hasSelectedSolutions = selectedSolutions.length > 0;
    
    if (hasSelectedSolutions) {
      // Use selected solutions
      ...
      return { 
        solutions: selectedSolutionObjects,
        contributingReasons: selectedContributingReasons
      };
    } else {
      // Use top 3 scoring solutions
                    ...
                    <div key={group} className="mb-6">
                      ...
                      <div className="grid grid-cols-1 gap-4">
                        {solutions.map((scoredSolution) => { // Iterate over scoredSolution objects
                          const { solution, score, trafficMatch, pickupMatch, contributingReasons } = scoredSolution; // Destructure
                          ...
                          return (
                            <SolutionCard
                              ...
                              pickupPreferenceMatch={pickupMatch} // Pass pickup preference match
                              userPickupPreference={businessParkInfo.employeePickupPreference} // Pass user preference
